...
AJS.$(document).ready(function () {
    ...
    /* milestone out limite */
    var case_limite = [14,14,14];
    ...
    function addMileStone(ms_extend,ms,past_d,future_d,current_d_line_position,range_d_ok_line_position,node_space_sum,initial_point,ctx){
        ...
        ctx.font = "10px Verdana";
        ctx.fillStyle = "#000000";
        ctx.fillText(tipodemilestone_label, ms.x-15, ms.y+22);
    ...
    function classificateMilestone(ms,past_d,future_d){
        ...
        if(compareDate(today,ms.enddate)||ms.statusmilestone.toLowerCase() == allcase_statusmilestone[0]){
            case_milestone = 1;
        }
        else if ((compareDate(ms.enddate,today) && compareDate(addDays(range_d_ok),ms.enddate))||(ms.enddate==today)||(ms.enddate==addDays(range_d_ok))){
            case_milestone = 2;
        }
        else if (compareDate(ms.enddate,addDays(range_d_ok))){
            case_milestone = 3;
        }
        return case_milestone;
    }
    function addIndexByMilestoneType(ms){
        var ms_extend = [[],[],[]];
        var milestone_type = [];
        var milestone_type_base = {name:'DEFAULT',count:0};
        milestone_type.push(milestone_type_base);

        for (var i = 0; i < ms.length; i++){
            for (var j = 0; j < ms[i].length; j++){
                var tipodemilestone_label = ms[i][j].tipodemilestone.substr(0, ms[i][j].tipodemilestone.indexOf(" - "));
                var milestone_type_found = 0;
                for (var k = 0; k < milestone_type.length; k++){
                    if(milestone_type[k].name == tipodemilestone_label){
                        milestone_type[k].count++;
                        ms_extend[i][j] = ("0" + milestone_type[k].count).slice(-2);
                        milestone_type_found = 1;
                        break;
                    }
                }
                if(milestone_type_found == 0){
                    milestone_type.push({name:tipodemilestone_label,count:1});
                    ms_extend[i][j] = "01";
                }
            }
        }
    ...
    function redraw(c,ctx) {
        ...
        var array_milestone_case_extend = addIndexByMilestoneType(array_milestone_case);
        //Se realiza el filtro para el caso milestone pasado.
        if(array_milestone_case[0].length>case_limite[0]){
            var array_milestone_case_extend_0 = [];
            var array_milestone_case_0 = [];
            var case_limite_0 = case_limite[0]-1;
            array_milestone_case_0.push(array_milestone_case[0][0]);
            for(var i = case_limite_0; i>0; i--)
                array_milestone_case_0.push(array_milestone_case[0][array_milestone_case[0].length-i]);
            array_milestone_case[0] = array_milestone_case_0;

            array_milestone_case_extend_0.push(array_milestone_case_extend[0][0]);
            for(var i = case_limite_0; i>0; i--)
                array_milestone_case_extend_0.push(array_milestone_case_extend[0][array_milestone_case_extend[0].length-i]);
            array_milestone_case_extend[0] = array_milestone_case_extend_0;
        }
        if(array_milestone_case[1].length>case_limite[1]){
            var array_milestone_case_extend_1 = [];
            var array_milestone_case_1 = [];
            for(var i = 0; i<case_limite[1]-1; i++)
                array_milestone_case_1.push(array_milestone_case[1][i]);
            array_milestone_case_1.push(array_milestone_case[1][array_milestone_case[1].length-1]);
            array_milestone_case[1] = array_milestone_case_1;

            for(var i = 0; i<case_limite[1]-1; i++)
                array_milestone_case_extend_1.push(array_milestone_case_extend[1][i]);
            array_milestone_case_extend_1.push(array_milestone_case_extend[1][array_milestone_case_extend[1].length-1]);
            array_milestone_case_extend[1] = array_milestone_case_extend_1;
        }
        if(array_milestone_case[2].length>case_limite[2]){
            var array_milestone_case_extend_2 = [];
            var array_milestone_case_2 = [];
            for(var i = 0; i<case_limite[2]-1; i++)
                array_milestone_case_2.push(array_milestone_case[2][i]);
            array_milestone_case_2.push(array_milestone_case[2][array_milestone_case[2].length-1]);
            array_milestone_case[2] = array_milestone_case_2;

            for(var i = 0; i<case_limite[2]-1; i++)
                array_milestone_case_extend_2.push(array_milestone_case_extend[2][i]);
            array_milestone_case_extend_2.push(array_milestone_case_extend[2][array_milestone_case_extend[2].length-1]);
            array_milestone_case_extend[2] = array_milestone_case_extend_2;
        }
        ...
        for (var i = 0; i<3; i++){
            ...
            for (var j = 0; j<array_milestone_case[i].length; j++){
                var ms_extend = "";
                if(array_milestone_case[i][j].tipodemilestone.substr(0, array_milestone_case[i][j].tipodemilestone.indexOf(" - ")) == "MS")
                  ms_extend = ValidateExtend(array_milestone_case[i][j].indicecontadormsgenerico);
                else
                  ms_extend = array_milestone_case_extend[i][j];
                switch(i){
                    case 0:
                        node_space_sum += node_space;
                        addMileStone(ms_extend,array_milestone_case[i][j],past_d,future_d,current_d_line_position,range_d_ok_line_position,node_space_sum,initial_point,context);
                        break;
                    case 1:
                        node_space_sum += node_space;
                        addMileStone(ms_extend,array_milestone_case[i][j],past_d,future_d,current_d_line_position,range_d_ok_line_position,node_space_sum,initial_point,context);
    ...
    function ConvertTwoDigitNumbers(n){
        var formattedNumber = ("0" + n).slice(-2);
        return formattedNumber;
    }
    function ValidateExtend(n){
        if (isNumeric(n) == true && (n.toString()).length == 1 )
            n = ConvertTwoDigitNumbers(Number(n));
        return n;
    }
